...
local GetVerisonBasmah = https.request('https://raw.githubusercontent.com/BasmahSource/BasmahFile/master/GetVersion.txt') or 0
print('\n\27[1;36m￤تم آدخآل آيدي آلمـطـور بنجآح , سـوف يتم تشـغيل آلسـورس آلآن .\n￤Success Save ID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
basmah = Token:match("(%d+)")
redis:mset(
basmah..":VERSION",GetVerisonBasmah,
...
redis:hset(basmah..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
info = {} 
...
https.request(Api_Token..'/sendMessage?chat_id='..SUDO_USER..'&text='..URL.escape(Text)..'&parse_mode=Markdow')
os.execute([[
rm -f ./README.md
rm -rf ./.git
chmod +x ./run
...
basmah = Token:match("(%d+)")
our_id = tonumber(basmah)
ApiBasmah = redis:get(basmah..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(basmah..":UserNameBot:")
SUDO_ID = tonumber(redis:get(basmah..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(basmah..'username:'..SUDO_ID).username
version = redis:get(basmah..":VERSION")
DataCenter = redis:get(basmah..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..basmah..')\027[0;32m\n'
...
..'¦ Api_Src->: \27[1;34m'..ApiBasmah..'\027[0;32m\n'
...
if not redis:get(basmah..":Check_Bot:"..W) then
Rgz,res=https.request(ApiBasmah..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(basmah..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(basmah..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(basmah..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(basmah..'group:add'..msg.chat_id_) then 
...
if msg.GroupActive and redis:get(basmah..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end
...
msg.Rank = 1
elseif redis:sismember(basmah..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(basmah..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ اساسي'
msg.TheRank = 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(basmah..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(basmah..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(basmah..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(basmah..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(basmah..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(basmah..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(basmah..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(basmah..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(basmah..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(basmah..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(basmah.."lock_Add"..msg.chat_id_) then
...
if msg.Special or msg.Admin then redis:srem(basmah..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(basmah..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(basmah..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Basmah then 
if msg.text and ScriptFile.iBasmah then
XBasmah = ScriptFile.Basmah
local list = redis:hgetall(basmah..":AwamerBotArray:"..msg.chat_id_)
for Basmah2,k in pairs(list) do
Text = msg.text
Text2 = k
if Text:match(Basmah2) then 
local amrr = {Text:match(Basmah2)}
local AmrOld = redis:hgetall(basmah..":AwamerBotArray2:"..msg.chat_id_)
amrnew = "" amrold = ""
...
GetMsg = ScriptFile.iBasmah(msg,{Text:match(Text2)})
if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(Basmah) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iBasmah(msg,{Text:match(Basmah)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Basmah.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dBasmah then
if ScriptFile.dBasmah(msg) == false then
return false
end
...
if Plug.Basmah then 
if msg.text and Plug.iBasmah then
for k, Basmah in pairs(Plug.Basmah) do
if msg.text:match(Basmah) then
local GetMsg = Plug.iBasmah(msg,{msg.text:match(Basmah)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Basmah.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dBasmah then
Plug.dBasmah(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source Basmah")
end 

end
else
print("The File Script.lua Not Runing in The Source Basmah")
end
end
...
function tdcli_update_callback(data)
	...
	redis:srem(basmah..'users',msg.sender_user_id_)
	end
...
if redis:get(basmah..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(basmah..":propin"..msg.chat_id_)
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) vardump(d)end,nil)   
	...
	if redis:get(basmah..'group:add'..data.chat_.id_) then
	redis:set(basmah..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	elseif data.ID == "UpdateChannel" then  
	if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
	if redis:get(basmah..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(basmah..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(basmah..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	basmah..'group:name'..k,IDS.Title,
	basmah..'num_msg_max'..k,5,
	basmah..'group:add'..k,true,
	basmah..'lock_link'..k,true,
	basmah..'lock_spam'..k,true,
	basmah..'lock_webpage'..k,true,
	basmah..'lock_markdown'..k,true,
	basmah..'lock_flood'..k,true,
	basmah..'lock_bots'..k,true,
	basmah..'mute_forward'..k,true,
	basmah..'mute_contact'..k,true,
	basmah..'mute_document'..k,true,
	basmah..'mute_inline'..k,true,
	basmah..'lock_username'..k,true,
	basmah..'replay'..k,true
	)
	redis:sadd(basmah..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(basmah..'username:'..ID,'username',user)
	redis:sadd(basmah..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(basmah..'username:'..ID,'username',user)
	redis:sadd(basmah..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(basmah..'username:'..ID,'username',user)
	redis:sadd(basmah..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(basmah..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(basmah..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(basmah..'username:'..data.user_.id_).username
	...
	redis:hset(basmah..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
